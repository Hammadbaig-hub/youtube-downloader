...
_SETTINGS_PATH = _config.ADMIN_SETTINGS_PATH
_SETTINGS_DEFAULTS = {
    "site_name":             "VidFlow",
...
def dashboard():
    ...
    top_platform_row = (
        db.session.query(Download.platform, func.count(Download.id))
...
def stats():
    ...
    top_users = (
        db.session.query(User.name, User.email, func.count(Download.id))
...
def stats_json():
    ...
    daily = (
        db.session.query(func.date(Download.date), func.count(Download.id))
        .filter(Download.date >= since)
        .group_by(func.date(Download.date))
        .order_by(func.date(Download.date))
        .all()
    )
    ...
    top_users = (
        db.session.query(User.name, func.count(Download.id))
        .join(Download, User.id == Download.user_id)
        .filter(Download.date >= since)
        .group_by(User.id)
        .order_by(func.count(Download.id).desc())
...
def settings():
    ...
    if request.method == "POST":
        if not _csrf_ok():
            ...
            try:
                cfg["max_downloads_per_day"] = int(
                    request.form.get("max_downloads_per_day") or 100
                )
                cfg["max_file_size_mb"] = int(
                    request.form.get("max_file_size_mb") or 2000
                )
            except ValueError:
                pass
